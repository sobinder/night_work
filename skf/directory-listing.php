...
$output[] = array(  'hours' => $hours, 'place' => $place, 'street' => $street, 'city' => $city, 'id' =>$post->ID, 'name' =>$post->post_title, 'lat' =>$location['lat'], 'lng' =>$location['lng'],'address' =>$location['address'], 'email' =>$email, 'phone' =>$phone, 'fax' =>$fax, 'category' =>'page','link'=>$link,'web'=>$google);
}
?>
...
<tbody><tr>
<td valign="top" align="left" width="45"><img src="<?php // echo cs_get_option('map_crown');?>"></td>
...
{{#if web}}Find us on <a href="{{web}}" target="_blank">Google Plus</a></td>{{/if}}
    ...
    <div class="col-sm-2 location-distance nmp text-left">
	...
	<div class="col-sm-3 nmp text-left">
	<h2>Store Hours</h2>
	...
	<div class="loc-link"><a href="{{link}}">View this location</a></div>
		...
		<div class="list-content hidden">
			...
			{{#if distance}}
				<div class="loc-dist hidden">{{distance}} {{length}}</div>
				<div class="loc-directions"><a href="https://maps.google.com/maps?saddr=&amp;daddr={{address}} {{address2}} {{city}}, {{state}} {{postal}}" target="_blank">Directions</a></div>
			{{/if}}
		</div>
	</div>
</li>
...
<div class="bh-sl-container locator-form">
 ...
 <select id="bh-sl-maxdistance" name="bh-sl-maxdistance" style="display:none;">
            ...
            &nbsp;&nbsp;<button id="bh-geo-submit" type="submit">Use my location instead</button>
            </div>
